...
func (k Keeper) Slash(ctx context.Context, consAddr sdk.ConsAddress, infractionHeight, power int64, slashFactor math.LegacyDec) (math.Int, error) {
	...
	if validator.Tokens.IsPositive() {
		...
		if err := k.Hooks().CustomBeforeValidatorSlashed(
			...
			return math.NewInt(0), fmt.Errorf("failed to call custom before validator slashed hook: %w", err)
		}
	}
...
func (k Keeper) SlashUnbondingDelegation(ctx context.Context, unbondingDelegation types.UnbondingDelegation,
	...
	if burnedAmount.IsPositive() {
		if err := k.Hooks().AfterUnbondingDelegationSlashed(
			...
			return math.NewInt(0), fmt.Errorf("failed to call after unbonding delegation slashed hook: %w", err)
		}
	}
...
func (k Keeper) SlashRedelegation(ctx context.Context, srcValidator types.Validator, redelegation types.Redelegation,
	...
	if burnedAmount.IsPositive() {
		if err := k.Hooks().AfterRedelegationSlashed(ctx, valDstAddr, delegatorAddress, burnedAmount); err != nil {
			return math.NewInt(0), fmt.Errorf("failed to call after redelegation slashed hook: %w", err)
		}
	}
